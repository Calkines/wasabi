...
public class DatabaseExperimentRepositoryTest {
    ...
    public void setup() {
        ...
        // verify(flyway,
        // atLeastOnce()).setLocations("com/intuit/wasabi/repository/impl/mysql/migration");
        ...
        // Experiment.ID id = Experiment.ID.newInstance();
        // List queryResult = mock(List.class);
        // when(transaction.select(anyString(), eq(appName.toString()),
        // eq(label.toString()),
        // eq(Experiment.State.DELETED.toString()))).thenReturn(queryResult);
        // when(queryResult.isEmpty()).thenReturn(true);
        ...
        // when(queryResult.isEmpty()).thenReturn(false);
        // when(queryResult.size()).thenReturn(2);
        // given(transaction.select(anyString(), eq(appName.toString()),
        // eq(label.toString()),
        ...
        // da.writeLong(id.getRawID().getLeastSignificantBits());
        // when(map.get("id")).thenReturn(ba.toByteArray());
        // when(transaction.select(anyString(),
        // eq(Experiment.State.DELETED.toString()))).thenReturn(queryResult);
        ...
        // exception")).when(transaction).select(anyString(),
        // eq(Experiment.State.DELETED.toString()));
        // BDDCatchException.when(repository).getExperiments();
        // BDDCatchException.then(caughtException())
        ...
        // DatabaseExperimentRepository repository = spy(new
        // DatabaseExperimentRepository(transactionFactory,
        ...
        // BDDCatchException.when(repository).getBucket(Experiment.ID.newInstance(),
        // Bucket.Label.valueOf("TestLabel"));
        // BDDCatchException.then(caughtException())
        // .isInstanceOf(UnsupportedOperationException.class)
        ...
        // Experiment.ID id = Experiment.ID.newInstance();
        // NewExperiment mockedNewExperiment =
        // NewExperiment.withID(id).withSamplingPercent(1.0)
        ...
        // doNothing().when(transaction).insert(anyString(), any());
        // Experiment.ID result = repository.createExperiment(mockedNewExperiment);
        // assertThat(result, is(id));

        // doThrow(new DatabaseException("Test")).when(transaction).insert(anyString(),
        // any());
        ...
        // doThrow(new RuntimeException("RTE")).when(transaction).insert(anyString(),
        // any());
        // BDDCatchException.when(repository).createExperiment(mockedNewExperiment);
        // BDDCatchException.then(caughtException())
        // .isInstanceOf(Exception.class)
        ...
        // when(transaction.update(anyString(), any())).thenReturn(2);
        ...
        // when(transaction.update(anyString(), any())).thenReturn(0);
        // BDDCatchException.when(repository).updateExperiment(mockedExperiment);
        // BDDCatchException.then(caughtException())
        // .isInstanceOf(RepositoryException.class)
        ...
        // doNothing().when(experimentValidator).validateExperiment(eq(mockedExperiment));
        // when(transaction.update(anyString(), any())).thenReturn(1);
        // Experiment result = repository.updateExperimentState(mockedExperiment,
        // state);
        // assertThat(result, is(mockedExperiment));

        // when(transaction.update(anyString(), any())).thenReturn(2);
        // BDDCatchException.when(repository).updateExperimentState(mockedExperiment,
        // state);
        // BDDCatchException.then(caughtException())
        // .isInstanceOf(RepositoryException.class)
        // .hasMessage("Concurrent updates; please retry")
        ...
        // doThrow(new DatabaseException("db
        // error")).when(transaction).insert(anyString(),
        ...
        // assertThat(bucket.getLabel().toString().trim().isEmpty(),
        // is(not(nullValue())));
        ...
        // when(transaction.update(anyString(), any())).thenReturn(1);
        // BucketList result = repository.updateBucketBatch(Experiment.ID.newInstance(),
        // bucketList);
        ...
        // when(transaction.update(anyString(), any())).thenReturn(0);
        // BDDCatchException.when(repository).deleteBucket(Experiment.ID.newInstance(),
        // Bucket.Label.valueOf("label"));
        // BDDCatchException.then(caughtException())
        ...
        // repository.logBucketChanges(Experiment.ID.newInstance(),
        // Bucket.Label.valueOf("lable"),
        // new ArrayList<Bucket.BucketAuditInfo>());
        // repository.logExperimentChanges(Experiment.ID.newInstance(),
        // new ArrayList<Experiment.ExperimentAuditInfo>());
        // }
        ...
        // when(transaction.select(anyString(), eq(id),
        // eq(DELETED.toString()))).thenReturn(firstQueryResult);
        ...
        // .hasMessageContaining("Experiment")
        // .hasNoCause();
        // verify(transaction, times(1)).select(anyString(), any());
