...
public class DatabaseAnalyticsTest {
    ...
    // when(transaction.select(anyString(), eq(experimentId), eq(true),
    // eq(rollupDate), anyString()))
    // .thenReturn(expected);
    // List<Map> result = databaseAnalytics.getRollupRows(experimentId, rollupDate,
    // parameters);
    // assertThat(result, is(expected));
    // // Exception is thrown in this case
    // doThrow(new RuntimeException())
    // .when(transaction)
    // .select(anyString(), eq(experimentId), eq(true), eq(rollupDate),
    // anyString());
    ...
    // List<Map> expected = mock(List.class);
    // when(transaction.select(anyString(), any())).thenReturn(expected);
    // List<Map> result = databaseAnalytics.getJointActions(experimentId,
    // parameters);
    // assertThat(result, is(expected));
    // // exception while select
    // doThrow(new RuntimeException()).when(transaction)
    ...
    // List<Map> expected = mock(List.class);
    // when(transaction.select(anyString(), any())).thenReturn(expected);
    // List<Map> result = databaseAnalytics.getImpressionRows(experimentId,
    // parameters);
    // assertThat(result, is(expected));
    // // exception while select
    // doThrow(new RuntimeException()).when(transaction)
    ...
    // Map<Bucket.Label, BucketCounts> result =
    // databaseAnalytics.getEmptyBuckets(experimentId);
    // assertThat(result.size(), is(1));
    // assertThat(result.get(Bucket.Label.valueOf("TEST_LABEL")).getLabel().toString(),
    // is("TEST_LABEL"));
    // assertThat(result.get(Bucket.Label.valueOf("TEST_LABEL")).getActionCounts().size(),
    // is(0));
    // // exception while select
    // doThrow(new RuntimeException()).when(transaction)
    // .select(anyString(), any());
    ...
    // when(parameters.getContext().getContext()).thenReturn("TEST");
    // when(transaction.select(anyString(), any())).thenReturn(expected);
    // List<Map> result = databaseAnalytics.getCountsFromRollups(experimentId,
    // parameters);
    // assertThat(result, is(expected));
    // // exception while select
    // doThrow(new RuntimeException()).when(transaction)
    ...
    // when(transaction.select(anyString(), eq(experimentId),
    // eq("TEST"))).thenReturn(queryResult);
    // when(queryResult.isEmpty()).thenReturn(true);
    // boolean result = databaseAnalytics.checkMostRecentRollup(experiment,
    // parameters, to);
