...
public class ExperimentDetailTest {
        ...
        private Experiment exp = Experiment.withID(expId).withApplicationName(appName)
                        .withModificationTime(modTime.getTime())
                        .withLabel(expLabel).withStartTime(startTime.getTime()).withState(Experiment.State.TERMINATED)
                        .build();
        ...
        // new ExperimentDetail(expId, Experiment.State.DELETED, expLabel, appName,
        // modTime.getTime(),
        // startTime.getTime(), endTime.getTime(), description, tags);
        // }
        ...
        // // check labels
        // List<Bucket.Label> labels =
        // buckets.stream().map(Bucket::getLabel).collect(Collectors.toList());
        ...
        // buckets.stream().map(Bucket::getAllocationPercent).collect(Collectors.toList());
        // assertEquals(percentage, expDetail.getBuckets().stream().
        // map(ExperimentDetail.BucketDetail::getAllocationPercent).
        // collect(Collectors.toList()));

        // // check isControl
        // List<Boolean> controls =
        // buckets.stream().map(Bucket::isControl).collect(Collectors.toList());
        ...
        // buckets.stream().map(Bucket::getDescription).collect(Collectors.toList());
