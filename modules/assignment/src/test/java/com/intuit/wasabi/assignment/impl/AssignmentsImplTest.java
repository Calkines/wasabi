...
public class AssignmentsImplTest {
        ...
        private Provider<Envelope<AssignmentEnvelopePayload, DatabaseExport>> assignmentDBEnvelopeProvider = mock(
                        Provider.class, RETURNS_DEEP_STUBS);
        private Provider<Envelope<AssignmentEnvelopePayload, WebExport>> assignmentWebEnvelopeProvider = mock(
                        Provider.class, RETURNS_DEEP_STUBS);
    ...
    public void testQueuesDetails() {
        ...
        public void testGetSingleAssignmentNullAssignmentExperimentNotFound() {
                ...
                bucketList.addBucket(
                                Bucket.newInstance(id, Bucket.Label.valueOf("red")).withAllocationPercent(0.5).build());
                bucketList.addBucket(Bucket.newInstance(id, Bucket.Label.valueOf("blue")).withAllocationPercent(0.5)
                                .build());
                ...
                when(metadataCache.getPrioritizedExperimentListMap(appName))
                                .thenReturn(prioritizedExperimentListOptional);
                ...
                Assignment result = assignmentsImpl.doSingleAssignment(user, appName, nonExistantLabel, context, true,
                                true,
        ...
        public void testGetSingleAssignmentNullAssignmentExperimentNoProfileMatch() throws IOException {
                ...
                doReturn(false).when(assignmentsImpl).doesProfileMatch(any(Experiment.class),
                                any(SegmentationProfile.class),
        ...
        public void testGetSingleAssignmentAssertExistingAssignment() {
                ...
                when(assignmentsRepository.getAssignments(user, appName, context, expMap))
        ...
        public void testGetSingleAssignmentProfileMatchAssertNewAssignment() throws IOException {
                AssignmentsImpl assignmentsImpl = spy(new AssignmentsImpl(new HashMap(),
                                experimentRepository, assignmentsRepository, ruleCache, pages, assignmentDecorator,
                                threadPoolExecutor,
                                eventLog, metadataCacheEnabled, metadataCache, experimentUtil));
                ...
                doReturn(true).when(assignmentsImpl).doesProfileMatch(any(Experiment.class),
                                any(SegmentationProfile.class),
                                any(HttpHeaders.class), any(Context.class));

                doReturn(assignment).when(assignmentsImpl).generateAssignment(any(Experiment.class), eq(user),
                                any(Context.class), any(Boolean.class), any(BucketList.class), any(Date.class),
                                any(SegmentationProfile.class));
        ...
        // SegmentationProfile.newInstance().build();
        //
        // //Check to show that there is no context parameter within the segmentation
        // profile
        ...
        // Table<Experiment.ID, Experiment.Label, String> result =
        // HashBasedTable.create();
        //
        // //Creating a calendar object that will help setup the Experiment Object
        // later.
        ...
        // Experiment.withID(Experiment.ID.newInstance()).withApplicationName(testApp).build();
        ...
        // Bucket redBucket = Bucket.newInstance(experiment.getID(),
        // Bucket.Label.valueOf("red")).withAllocationPercent(1.0).build();
        ...
        // Mockito.when(assignmentsRepository.getAssignments(User.ID.valueOf("user-a"),
        // testApp, context, allExperiments)).thenReturn(result);
        ...
        //
        // cassandraAssignments = null; //new AssignmentsImpl(cassandraRepository,
        // assignmentsRepository, mutexRepository, random,
        //// ruleCache, pages, priorities, assignmentDBEnvelopeProvider,
        // assignmentWebEnvelopeProvider, null, // FIXME
        //// decisionEngineScheme, decisionEngineHost, decisionEnginePath,
        //// decisionEngineReadTimeOut, decisionEngineConnectionTimeOut,
        // decisionEngineUseProxy,
        //// decisionEngineUseConnectionPooling, decisionEngineMaxConnectionsPerHost,
        // proxyPort, proxyHost, eventLog);
        //
        // //Pass the segmentation profile to be modified within getAssignment to have
        // context parameter as an additional attribute.
        // cassandraAssignments.getAssignment(User.ID.valueOf("user-a"), testApp,
        // experiment.getLabel(), context, true, false, segmentationProfile, null);
        //
        // //Check to show that there is now a context parameter within the segmentation
        // profile
        // assertTrue(segmentationProfile.hasAttribute("context"));
        //
        // //Check to see that the context parameter within the segmentation profile has
        // the correct value
        // assertEquals("ERROR: The context parameter passed in did NOT match with the
        // context added to the segmentation profile. ", context.getContext(),
        // segmentationProfile.getAttribute("context"));
        // }
        ...
        // experiment.setEndTime(c.getTime());
        // experiment.setState(Experiment.State.RUNNING);
        // assignment = cassandraAssignments.getAssignment(User.ID.valueOf("user-a"),
        // testApp,
        // experiment.getLabel(), context, true, false, null, null);
        ...
        ////
        //// @Override
        //// protected Experiment getExperimentFromTable(Table<Experiment.ID,
        // Experiment.Label, Experiment> allExperiments,
        ...
        public void getAssignment_test_3() throws IOException {
                ...
                c.setTime(new Date());
                c.add(Calendar.DATE, -1);
                final Experiment experiment = Experiment.withID(Experiment.ID.newInstance())
                                .withApplicationName(testApp)
                                .withLabel(Experiment.Label.valueOf("exp")).withStartTime(c.getTime())
                                .withSamplingPercent(1.0).build();
                Experiment experiment2 = Experiment.withID(Experiment.ID.newInstance()).withApplicationName(testApp)
                                .withLabel(Experiment.Label.valueOf("exp2")).withStartTime(c.getTime())
                                .withSamplingPercent(1.0).build();
                c.add(Calendar.DATE, 10);
                experiment.setEndTime(c.getTime());
                ...
                Bucket greenBucket = Bucket.newInstance(experiment2.getID(), Bucket.Label.valueOf("green"))
                ...
                // Mockito.when(assignmentsRepository.getAssignments(User.ID.valueOf("user-b"),
                // testApp, context, allExperiments)).thenReturn(result);
                ...
                Assignment newAssignment = Assignment.newInstance(assignment.getExperimentID())
                                ...
                                .withCacheable(null)
                                .build();
                Mockito.when(assignmentsRepository.getAssignment(User.ID.valueOf("user-a"), testApp, experiment.getID(),
                                context)).thenReturn(newAssignment);

                assignment = assignmentsRepository.getAssignment(User.ID.valueOf("user-a"), testApp, experiment.getID(),
                                context);
                ...
                newAssignment = Assignment.newInstance(assignment.getExperimentID())
                                ...
                                .withCacheable(null)
                                .build();
                Mockito.when(assignmentsRepository.getAssignment(User.ID.valueOf("user-b"), testApp, experiment.getID(),
                                context)).thenReturn(newAssignment);

                assignment = assignmentsRepository.getAssignment(User.ID.valueOf("user-b"), testApp, experiment.getID(),
                                context);
                assert assignment.getBucketLabel() == null;
                assert assignment.getStatus() == Assignment.Status.NEW_ASSIGNMENT;
                ...
                newAssignment = Assignment.newInstance(assignment.getExperimentID())
                                ...
                                .withCacheable(null)
                                .build();
                Mockito.when(assignmentsRepository.getAssignment(User.ID.valueOf("user-c"), testApp, experiment.getID(),
                                context)).thenReturn(newAssignment);
                assignment = assignmentsRepository.getAssignment(User.ID.valueOf("user-c"), testApp, experiment.getID(),
                                context);
         ...
         * AssignmentsImpl impl = new AssignmentsImpl(assignmentsRepository,
         * mutexRepository);
         ...
         * .
         * build();
         * given(mutexRepository.getExclusions(isA(Experiment.ID.class))).willReturn(
         * list);
         * given(assignmentsRepository.getUserAssignments(isA(User.ID.class),
         * isA(Name.class), (Context) isNull()))
         * .willReturn(emptyIds);
         ...
         * 
         * @Test
         * public void checkMutexWithOneExclusionListButAssignmentEmptyStateRunning()
         * throws Exception {
         * AssignmentsImpl impl = new AssignmentsImpl(assignmentsRepository,
         * mutexRepository);
         * Experiment experiment =
         ...
         * 
         * @Test
         * public void checkMutexWithOneExclusionListAssignmentSameStateRunning() throws
         * Exception {
         * AssignmentsImpl impl = new AssignmentsImpl(assignmentsRepository,
         * mutexRepository);
         * Experiment experiment =
         ...
         * 
         * @Test
         * public void checkMutexWithOneExclusionListAssignmentDeletedStateRunning()
         * throws Exception {
         * AssignmentsImpl impl = new AssignmentsImpl(assignmentsRepository,
         * mutexRepository);
         * Experiment experiment =
        ...
        public void getAssignment_test_4() throws IOException {
                ...
                final Experiment experiment = Experiment.withID(Experiment.ID.newInstance())
                                .withApplicationName(testApp)
                                .withLabel(Experiment.Label.valueOf("exp_DE1")).withStartTime(c.getTime())
                                .withSamplingPercent(1.0).withIsPersonalizationEnabled(true).build();
                c.add(Calendar.DATE, 10);
                experiment.setEndTime(c.getTime());
        ...
        public void createAssignmentObjectTest() throws IOException {
                ...
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("bucket-1"))
                                .withAllocationPercent(0.9d).withPayload("bucket1").withState(Bucket.State.OPEN)
                                .build());
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("bucket-2"))
                                .withAllocationPercent(0.1d).withPayload("bucket-2").withState(Bucket.State.OPEN)
                                .build());
                ...
                // --------- Make actual call
                Assignment newAssignment = assignmentsImpl.createAssignmentObject(exp1, userID, context, selectBucket,
                                bucketList1, date, segmentationProfile);
        ...
        public void createAssignmentObjectTestForNoOpenBucket() throws IOException {
                ...
                BucketList bucketList1 = new BucketList();
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("bucket-1"))
                                .withAllocationPercent(0.0d).withPayload("bucket1").withState(Bucket.State.CLOSED)
                                .build());
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("bucket-2"))
                                .withAllocationPercent(0.0d).withPayload("bucket-2").withState(Bucket.State.CLOSED)
                                .build());
        ...
        public void doBatchAssignmentsAllNewAssignmentTest() throws IOException {
                ...
                BucketList bucketList1 = new BucketList();
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("red"))
                                .withAllocationPercent(0.5).build());
                bucketList1.addBucket(Bucket.newInstance(exp1.getID(), Bucket.Label.valueOf("blue"))
                                .withAllocationPercent(0.5).build());
                ...
                Mockito.when(assignmentsRepository.getAssignments(user, appName, context, expMap))
                                .thenReturn(noExistingAssignments);
        ...
        public void putAssignment_test() throws IOException {
                ...
                Mockito.when(assignmentsRepository.getAssignment(User.ID.valueOf("user-b"), testApp, experiment.getID(),
                                context)).thenReturn(newAssignment);
                Mockito.doNothing().when(assignmentsRepository).deleteAssignment(experiment, User.ID.valueOf("user-b"),
                                context, testApp, assignment);
                ...
                Mockito.when(cassandraAssignments.putAssignment(userA, testApp, expLabel, context, redBucket.getLabel(),
                                true)).thenReturn(newAssignment);
                ...
                Mockito.when(cassandraAssignments.putAssignment(userA, testApp, expLabel, context, redBucket.getLabel(),
                                false)).thenReturn(newAssignment);
                assert newAssignment.getStatus() == Assignment.Status.NEW_ASSIGNMENT;
                assert newAssignment.getBucketLabel().toString().equals(redBucket.getLabel().toString());
                ...
                Mockito.when(cassandraAssignments.putAssignment(userA, testApp, expLabel, context, null, true))
        ...
        private Experiment setupMocksforRapidExperiment(
                ...
                BucketList bucketList = new BucketList();
                bucketList.addBucket(Bucket.newInstance(experimentId, Bucket.Label.valueOf("red"))
                                .withAllocationPercent(0.5).build());
                bucketList.addBucket(Bucket.newInstance(experimentId, Bucket.Label.valueOf("blue"))
                                .withAllocationPercent(0.5).build());
        ...
        private static Experiment createMockExperiment(Experiment.ID experimentId, Experiment.Label exprimentLabel,
        ...
        public void testExperimentFetchSameNameButDeleted() {
                ...
                Experiment deletedExperiment = createMockExperiment(deletedExperimentId, commonExperimentLabel,
                                Experiment.State.DELETED);
                Experiment runningExperiment = createMockExperiment(runningExperimentId, commonExperimentLabel,
        ...
        public void testExperimentFetchPrioritizedListEmpty() {
                ...
                Experiment deletedExperiment = createMockExperiment(deletedExperimentId, experimentLabel,
                                Experiment.State.DELETED);

                Optional<PrioritizedExperimentList> prioritizedExperimentListOptional = Optional
                                .of(new PrioritizedExperimentList());
        ...
        // Experiment.withID(Experiment.ID.newInstance()).withApplicationName(testApp)
         ...
         * {@link AssignmentsImpl#mergePersonalizationResponseWithSegmentation(SegmentationProfile, PersonalizationEngineResponse)}
         */
        // FIXME:
        // @Test
        // @Ignore("FIXME")
        ...
        // PersonalizationEngineResponse persRes =
        // mock(PersonalizationEngineResponse.class);
        ...
        //
        // assignmentsImpl.mergePersonalizationResponseWithSegmentation(segmentationProfile,
        // persRes);
        ...
        // }
        /*
         * FIXME: Traffic Analyzer change commented for Datastax-driver-migration
         * release...
         * 
         * @Test
         * public void testGetExperimentAssignmentRatioPerDay() {
         * Experiment experiment1 =
         * Experiment.withID(Experiment.ID.newInstance()).build();
         ...
         * 
         * doReturn(map1).when(assignmentsRepository).
         * getExperimentBucketAssignmentRatioPerDay(experiment1.getID(),
         * OffsetDateTime.MIN, OffsetDateTime.MAX);
         * doReturn(map2).when(assignmentsRepository).
         * getExperimentBucketAssignmentRatioPerDay(experiment2.getID(),
         * OffsetDateTime.MIN, OffsetDateTime.MAX);
         * doReturn(map3).when(assignmentsRepository).
         * getExperimentBucketAssignmentRatioPerDay(experiment3.getID(),
         * OffsetDateTime.MIN, OffsetDateTime.MAX);
         * 
         ...
         * 
         * Map<Experiment.ID, Map<OffsetDateTime, Double>> actual =
         * assignmentsImpl.getExperimentAssignmentRatioPerDay(experiments,
         * OffsetDateTime.MIN, OffsetDateTime.MAX);
         * Assert.assertEquals("T1 Should exactly return two items.", 2, actual.size());
         * Assert.assertEquals("T1 EntrySet for ID 1 should contain 1 element", 1,
         * actual.get(experiment1.getID()).size());
         * Assert.assertEquals("T1 EntrySet for ID 1 should contain map1", map1,
         * actual.get(experiment1.getID()));
         * Assert.assertEquals("T1 EntrySet for ID 2 should contain 1 element", 1,
         * actual.get(experiment2.getID()).size());
         * Assert.assertEquals("T1 EntrySet for ID 2 should contain map2", map2,
         * actual.get(experiment2.getID()));
         * 
         * experiments.add(experiment3);
         * actual = assignmentsImpl.getExperimentAssignmentRatioPerDay(experiments,
         * OffsetDateTime.MIN, OffsetDateTime.MAX);
         * Assert.assertEquals("T2 Should exactly return three items.", 3,
         * actual.size());
         * Assert.assertEquals("T2 EntrySet for ID 1 should contain 1 element", 1,
         * actual.get(experiment1.getID()).size());
         * Assert.assertEquals("T2 EntrySet for ID 1 should contain map1", map1,
         * actual.get(experiment1.getID()));
         * Assert.assertEquals("T2 EntrySet for ID 2 should contain 1 element", 1,
         * actual.get(experiment2.getID()).size());
         * Assert.assertEquals("T2 EntrySet for ID 2 should contain map2", map2,
         * actual.get(experiment2.getID()));
         * Assert.assertEquals("T2 EntrySet for ID 3 should contain 1 element", 1,
         * actual.get(experiment3.getID()).size());
         * Assert.assertEquals("T2 EntrySet for ID 3 should contain map3", map3,
         * actual.get(experiment3.getID()));
         * }
         * 
         * FIXME: Traffic Analyzer change commented for Datastax-driver-migration
         * release...
         ...
         * for (int i = 1; i <= 10; ++i) {
         * OffsetDateTime odt = OffsetDateTime.of(2000, 1, i, 0, 0, 0, 0,
         * ZoneOffset.UTC);
         ...
         * }
         * 
         * // Prepare list of experiments and their priorities, and set up
         * assignmentsRepository to return the right maps
         * List<Experiment> experiments = new ArrayList<>();
         ...
         * priorities.put(experiment.getID(), (i + 1) % 5 + 1); // results in
         * priorities: 3, 4, 5, 1, 2
         * doReturn(assignmentRatios).when(assignmentsRepository).
         * getExperimentBucketAssignmentRatioPerDay(experiment.getID(),
         * offsetDateTimes.get(0), offsetDateTimes.get(9));
         * doReturn(assignmentRatiosPart).when(assignmentsRepository).
         * getExperimentBucketAssignmentRatioPerDay(experiment.getID(),
         * offsetDateTimes.get(3), offsetDateTimes.get(6));
         * }
         * 
         * // query and check results: with all data
         * List<ImmutableMap<String, ?>> results = new ArrayList<>(2);
         * results.add(assignmentsImpl.getExperimentAssignmentRatioPerDayTable(
         * experiments, priorities, offsetDateTimes.get(0), offsetDateTimes.get(9)));
         * results.add(assignmentsImpl.getExperimentAssignmentRatioPerDayTable(
         * experiments, priorities, offsetDateTimes.get(3), offsetDateTimes.get(6)));
         * for (int i = 0; i < results.size(); ++i) {
         * ImmutableMap result = results.get(i);
         * Assert.assertTrue("Result must contain key 'experiments'",
         * result.containsKey("experiments"));
         * Assert.assertTrue("Result must contain a list for key 'experiments'",
         * result.get("experiments") instanceof List);
         * 
         * Assert.assertTrue("Result must contain key 'priorities'",
         * result.containsKey("priorities"));
         * Assert.assertTrue("Result must contain a list for key 'priorities'",
         * result.get("priorities") instanceof List);
         * 
         * Assert.assertTrue("Result must contain key 'samplingPercentages'",
         * result.containsKey("samplingPercentages"));
         * Assert.assertTrue("Result must contain a list for key 'samplingPercentages'",
         * result.get("samplingPercentages") instanceof List);
         * 
         * Assert.assertTrue("Result must contain key 'assignmentRatios'",
         * result.containsKey("assignmentRatios"));
         * Assert.assertTrue("Result must contain a list for key 'assignmentRatios'",
         * result.get("assignmentRatios") instanceof List);
         * 
         * Assert.assertEquals("List 'experiments' must contain 5 items.", 5, ((List)
         * result.get("experiments")).size());
         * Assert.assertEquals("List 'priorities' must contain 5 items.", 5, ((List)
         * result.get("priorities")).size());
         * Assert.assertEquals("List 'samplingPercentages' must contain 5 items.", 5,
         * ((List) result.get("samplingPercentages")).size());
         * Assert.assertEquals(String.
         * format("List 'assignmentRatios' must contain %d items.", i == 0 ? 10 : 4), i
         * == 0 ? 10 : 4, ((List) result.get("assignmentRatios")).size());
         * 
         * @SuppressWarnings("unchecked")
         * List<Map<String, ?>> assignmentRatiosResult = (List<Map<String, ?>>)
         * result.get("assignmentRatios");
         * 
         * Assert.assertTrue(String.format("ratios do not contain 'date' keys (i == %d)"
         * , i),
         * assignmentRatiosResult.stream()
         * .allMatch(m -> m.containsKey("date")));
         * Assert.assertTrue(String.format("ratios do not contain all dates (i == %d)",
         * i),
         * assignmentRatiosResult.stream()
         * .map(m -> m.get("date"))
         ...
         * format("ratios do not contain 'values' keys (i == %d)", i),
         * assignmentRatiosResult.stream()
         * .allMatch(m -> m.containsKey("values")));
         * Assert.assertTrue(String.
         * format("ratios should always have 5 values (i == %d)", i),
         * assignmentRatiosResult.stream()
